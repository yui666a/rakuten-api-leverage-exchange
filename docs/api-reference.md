...
- `data` は必須（主時間足CSV）
- `sort` はサポートせず、結果保存後に `GET /backtest/results` で確認する

### GET /backtest/results?limit=20&offset=0&sort=created_at:desc

保存済みバックテスト結果の一覧を返す。

- ソートは `created_at:desc` 固定（他値は `400`）

### GET /backtest/results/:id

指定 ID のバックテスト結果（サマリー + トレード一覧）を返す。
