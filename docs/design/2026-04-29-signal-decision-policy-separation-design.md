...
- 作成日: 2026-04-29
- 著者: 共同 (Daisy + Claude)
- ステータス: **Phase 1 完了 (2026-05-02)**
- 関連 Issue: #221 (BookGate disabled)、`MEMORY.md` の "/status running は嘘をつく"
- 関連 PR (Phase 1):
  - PR1 #232 — entity 型 + decision_log migration (`dea4060`)
  - PR2 #233 — DecisionHandler shadow route (`b404eab`)
  - PR3 #234 — RiskHandler decision cutover + EntryCooldown (`b35b187`)
  - PR4 #235 — BookGate enable + EntryCooldown via profile (`0db7fee`)
  - PR5 #236 — UI surface + docs cleanup (`b8ebbc6`)
...
**重要 (2026-05-03 検証で判明)**: 当初の sweep 候補値はいずれも **PT15M backtest では効果検証不能** であることが判明した。

- **`entry_cooldown_sec`**: 当初 30/60/120/300 を候補にしていたが、StrategyHandler が IndicatorEvent (バー終値) でしか signal 評価しない設計のため、cooldown_sec < bar_interval (900) では「次バー終値時にとっくに expire」となり一度も発動しない。
  - **新しい候補**: `900 / 1800 / 3600 / 7200`（1〜8 バー分）。`production_ltc_60k` はとりあえず 1800 (= 2 バー分) で起動。
- **`max_slippage_bps` / `max_book_side_pct`**: backtest runner は BookSource (orderbook snapshot) が無い限り BookGate をアタッチしない (`backend/internal/usecase/backtest/runner.go:259`)。CSV-only backtest では値を変えても **挙動は完全に同じ**。
  - **検証経路**: \`slippageModel="orderbook"\` + persisted L2 snapshots を使った backtest、または live 観測のみが意味を持つ。CSV sweep は時間の無駄。

**更新後の sweep 候補**:
- `entry_cooldown_sec`: 900 / 1800 / 3600 / 7200 (PT15M 想定)
- `max_slippage_bps`: 10 / 15 / 20 / 30 (orderbook backtest or live でのみ効く)
- `max_book_side_pct`: 10 / 20 / 30 (同上)
