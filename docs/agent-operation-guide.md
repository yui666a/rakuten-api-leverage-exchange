...
- **自動売買パイプライン**: イベント駆動 (`EventDrivenPipeline`)。プライマリ足 (PT15M) が新しく確定するごとにテクニカル指標を計算し、ルールベースの Stance に基づいて売買シグナルを生成・実行する（=エントリー判断は約15分に1回）。保有中ポジションの SL/TP/Trailing は tick 単位で常時監視。残高/ポジション同期は別タイマー（`STATE_SYNC_INTERVAL_SEC` 既定 15 秒、ポジション 30 秒）で並走。
